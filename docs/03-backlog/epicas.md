...
## EP-012 � Fase 2: Persistencia, Cach� y Auto-Aprendizaje
...
| M�trica de �xito | Cuotas y logs persisten en PostgreSQL sin latencia s�ncrona; respuestas repetidas se sirven en < 20ms desde la Semantic Cache; Router ajusta pesos din�micamente. |

**De qu� se trata**: Implementaci�n de los motores de persistencia e inteligencia avanzada de la Fase 2/3. 

**Capabilities que agrupa**:
- **Persistencia Real**: Conexi�n a PostgreSQL para persistir el Quota Manager y la Auditor�a (abandonando la memoria ef�mera).
- **Cach� Sem�ntica**: Integraci�n con base de datos vectorial (ej. pgvector/Milvus) para evitar llamadas redundantes y ahorrar costos.
- **Learning Engine**: M�dulo que consume la telemetr�a hist�rica y ajusta los pesos de scoring autom�ticamente (auto-ruteo inteligente).
...
- HU-063 � Learning Engine: Loop de retroalimentaci�n de m�tricas a Router
- HU-064 � Cache Invalidator para propagar cambios de DB a RAM

---

## EP-013 � Fase 2: Plataforma Aut�noma y Gobernanza de Proveedores (Dashboard)
...
| M�trica de �xito | Administrador puede crear tokens de proveedores desde el dashboard; puede visualizar la cuota y gasto exacto por modelo en tiempo real. |

**De qu� se trata**: Dotar al UI (Dashboard) de capacidades completas de administraci�n para que la plataforma sea 100% aut�noma, sin depender de archivos YAML editados manualmente.

**Capabilities que agrupa**:
- **Gesti�n de Tokens (Credentials Manager)**: Crear, rotar y eliminar API Keys de proveedores externos (OpenAI, Anthropic, etc.) directamente desde la interfaz.
...
- **Gesti�n de Tenants y Agentes**: Creaci�n y revocaci�n de API Keys de los clientes/consumidores desde el Dashboard.
- **Alertas Proactivas**: Configuraci�n de umbrales para notificar (Slack/Email) cuando una cuota se acerque al l�mite.
